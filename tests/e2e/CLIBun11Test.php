...
final class CLIBun11Test extends Base
{
    ...
    protected array $build = [
        'docker run --rm -v $(pwd):/app -w /app/tests/e2e/sdks/cli oven/bun:1.1 bun install',
        'docker run --rm -v $(pwd):/app -w /app/tests/e2e/sdks/cli oven/bun:1.1 bun run build:types',
        'docker run --rm -v $(pwd):/app -w /app/tests/e2e/sdks/cli oven/bun:1.1 bun run build:lib:runtime',
        'docker run --rm -v $(pwd):/app -w /app/tests/e2e/sdks/cli oven/bun:1.1 bun run build:cli',
        'cp tests/e2e/languages/cli/test.js tests/e2e/sdks/cli/test.js'
    ];
    #[Override]
    protected string $command =
        'docker run --network="mockapi" --rm -v $(pwd):/app -w /app/tests/e2e/sdks/cli oven/bun:1.1 bun test.js';
